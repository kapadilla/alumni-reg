  ...
  <div v-if="applicant" class="flex flex-col h-full bg-surface">
    ...
    <div class="p-6 bg-surface border-b border-border shrink-0">
      ...
      <div class="flex items-center gap-4 mb-4">
        <div
          class="size-14 rounded-full bg-red-500/10 flex items-center justify-center text-red-500 shrink-0"
        >
          <Icon name="material-symbols:person-cancel" class="size-8" />
        </div>
        <div>
          <h4 class="font-semibold text-text text-xl">
            {{ fullName }}
          </h4>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div>
              ...
              <div class="font-medium">
                <template v-if="applicant.academicStatus.campus">
                  {{ applicant.academicStatus.campus }}
                </template>
            ...
            <div>
              <label class="text-xs text-subtle block mb-1">Year</label>
              <div class="font-medium">
                <template v-if="applicant.academicStatus.yearGraduated">
                  {{ applicant.academicStatus.yearGraduated }}
                </template>
                <span v-else class="text-subtle italic">N/A</span>
              </div>
            </div>
            <div v-if="applicant.academicStatus.studentNumber">
              <label class="text-xs text-subtle block mb-1"
                >Student Number</label
              >
              <div class="font-medium text-text">
                {{ applicant.academicStatus.studentNumber }}
              </div>
            </div>
          </div>
        </section>
              ...
              <div>
                ...
                <div class="mt-4 flex flex-wrap gap-4 text-xs text-subtle">
                  <div v-if="rejectionEntry?.performedByName">
                    ...
                    {{ rejectionEntry.performedByName }}
                  </div>
                  <div>
                    <span class="font-medium">Date:</span>
                    {{
                      formatDateTime(
                        applicant.rejectedAt || rejectionEntry?.timestamp,
                      )
                    }}
                  </div>
                </div>
            ...
            >
              ...
              <div class="flex flex-col items-center">
                <div
                  ...
                  :class="
                    entry.action === 'rejected' ? 'bg-red-500' : 'bg-primary/30'
                  "
                />
                <div
              ...
              <div class="flex-1 pb-6">
                ...
                >
                  <div>
                    ...
                    <p
                      v-if="entry.notes && entry.action !== 'rejected'"
    ...
    <!-- Footer -->
    <div
      class="p-4 border-t border-border bg-surface shrink-0 flex justify-end"
    >
      <button
