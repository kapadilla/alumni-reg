...
const confirmRevoke = async (data: { reason: string; notes?: string }) => {
  ...
  const success = await revokeMembership(
    revokingMember.value.id,
    data.reason,
    data.notes,
  );
  if (success) {
...
const openReinstateDialog = (member: {
  id: number;
  name: string;
  email: string;
  memberSince: string;
...
<template>
  <div class="p-4 md:p-8 w-full">
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-2xl md:text-3xl font-bold text-text mb-2">Members</h1>
    ...
    >
      ...
      >
        <template #prepend-filters>
          <div
            class="inline-flex items-center bg-background rounded-lg p-1 w-full md:w-auto"
          >
            <button
        ...
        <table class="w-full min-w-[900px]">
          ...
          <tbody class="divide-y divide-border">
            ...
            >
              ...
              <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-text">
                <template v-if="member.yearGraduated">{{
    ...
    <!-- Scroll to Top -->
    <div v-if="!loading && members.length > 0" class="flex justify-center py-6">
      <button
          ...
          <template v-if="viewingMember">
            ...
            <button
              ...
              @click="
                ...
                openReinstateDialog({
                  id: viewingMember.id,
                  name: getMemberName(viewingMember),
                  email: viewingMember.personalDetails.email,
                  memberSince: viewingMember.memberSince,
                });
              "
            >
